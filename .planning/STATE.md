...
- **Rà soát hàng ngang sau khi chạy app thật (29/08/2026) — thêm 4 lỗi cùng họ** (commit `5134b33`, `e2d6563`, `3fac9fb`, `ea61e00`): (4) `WalletRef.id`/`CategoryRef.id` ép `as String` không nullable trong khi `GET /reports/home` cố ý trả bản rút gọn không có `id` (đúng api/06 mục 1) → sập cả màn Tổng quan; (5) `/budgets/summary` trả `period: null` khi chưa có ngân sách, lệch api/05 mục 3 → màn Ngân sách trắng với MỌI tài khoản mới đăng ký; (6) mọi lỗi client (thiếu tham số bắt buộc, UUID/ngày/số sai định dạng) trả 500 thay vì 400 `VALIDATION_ERROR`; (7) message validation lộ tiếng Anh (`must be a well-formed email address`) và tên trường camelCase, cả hai lệch api/00 mục 4.4. Đã dựng `integration_test/real_backend/contract_parse_test.dart` chặn tái diễn: 11 endpoint × 2 trạng thái (có dữ liệu / vừa đăng ký) + 4 phép thử mã lỗi. Trạng thái "vừa đăng ký" là ca bắt được lỗi (5) mà trạng thái có dữ liệu không thấy.
- **Lỗi thứ tự khai báo route (commit `2a62ad8`)** — `/so-giao-dich/sua` và `/so-giao-dich/:id` trùng khuôn đường dẫn; `go_router` khớp theo THỨ TỰ KHAI BÁO nên route `:id` đứng trước nuốt mất route sửa, dựng `TransactionDetailScreen(id: "sua")` và gọi `GET /transactions/sua`. Màn Sửa giao dịch **chưa bao giờ mở được**. Không unit test hay `flutter analyze` nào thấy: cả hai route đều hợp lệ, chỉ thứ tự giữa chúng sai. Quy tắc: route tĩnh LUÔN khai trước route có tham số cùng tiền tố. Đã rà soát toàn bộ `app_routes.dart` — đây là cặp xung đột duy nhất.
