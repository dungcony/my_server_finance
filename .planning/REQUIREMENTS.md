...
- [ ] **CORE-07**: Lưu/trả timestamp UTC; gom nhóm báo cáo theo ngày/tháng tính theo giờ Việt Nam (UTC+7)
...
- [ ] **REPORT-01**: Mọi endpoint báo cáo tuân thủ 3 nguyên tắc bắt buộc: loại `type=transfer`, cộng gộp danh mục con vào cha, tính theo giờ Việt Nam UTC+7
...
- [ ] **AI-04**: User duyệt một hoặc nhiều bản nháp (`approve`, `approve-bulk` tối đa 20) — phần user sửa được ưu tiên, tạo giao dịch thật, cập nhật số dư, **bắt buộc ghi `user_corrections`** (diff AI vs người dùng chọn)
...
- [ ] **DEBT-03**: User ghi một lần trả nợ — tạo giao dịch thật (Thu nợ/Trả nợ), cộng dồn `paid_amount`, tự chuyển `status=settled` khi trả đủ; không sửa được `principal_amount`/`type`/`wallet_id`
- [ ] **DEBT-04**: User huỷ một lần trả nợ — hoàn tác giao dịch, trừ lại `paid_amount`, mở lại `outstanding` nếu đang settled
...
- [ ] **GOAL-03**: User nạp tiền vào mục tiêu — hai chế độ: chuyển tiền thật (transfer, yêu cầu goal có `wallet_id`) hoặc chỉ ghi nhận tiến độ; tự chuyển `status=completed` khi đạt/vượt target
- [ ] **GOAL-04**: User rút lại một lần nạp — hoàn tác giao dịch liên quan nếu có, trừ lại `saved_amount`, mở lại `in_progress` nếu không còn đủ target
