...
- [x] **AUTH-01
**: User đăng ký bằng email/password, server tự băm mật khẩu (bcrypt ≥12 hoặc argon2id), tự tạo ví "Tiền mặt" số dư 0, sinh cặp access/refresh token
- [x] **AUTH-02
**: User đăng nhập, sai email hoặc sai mật khẩu trả cùng mã lỗi `INVALID_CREDENTIALS` để tránh dò email
- [x] **AUTH-03
...
- [x] **WALLET-01
**: User xem danh sách ví của mình (lọc theo `type`, `only_in_total`, `include_shared`) và tổng tài sản (tách riêng cá nhân/chung, không cộng đôi)
- [x] **WALLET-02
**: User tạo ví (tên không trùng, `current_balance` = `initial_balance` lúc tạo, không sinh giao dịch nào)
- [x] **WALLET-03
**: User sửa ví — không sửa được `current_balance` và `type` qua PATCH (chỉ đổi qua giao dịch)
- [x] **WALLET-04
**: User xoá ví (mềm) — chặn nếu còn giao dịch và không xác nhận `delete_transactions`, không xoá được ví cuối cùng
- [x] **WALLET-05
**: User sắp xếp lại thứ tự hiển thị ví
- [x] **WALLET-06
**: User chuyển tiền giữa 2 ví trong 1 giao dịch DB — tạo đúng một bản ghi `type=transfer` với cả `wallet_id` và `destination_wallet_id`, mặc định cho phép chuyển dù không đủ số dư trừ khi `fail_if_insufficient=true`
- [x] **WALLET-07
...
- [x] **TXN-01**: User xem danh sách giao dịch có phân trang, lọc đầy đủ (ngày, loại, ví, danh mục, nguồn, từ khoá, khoảng tiền, có/không transfer), kèm `summary` tính trên toàn bộ kết quả lọc và loại trừ transfer
- [x] **TXN-02**: User xem giao dịch gom theo ngày (`by-date`) và chi tiết một giao dịch (kèm `related_debt`/`recurring`/`ai_drafts` nếu có)
...
- [x] **TXN-04**: User tạo tối đa 50 giao dịch cùng lúc (`bulk`) — dòng lỗi bị bỏ qua và báo trong `row_errors`, không từ chối toàn bộ
...
- [x] **TXN-08**: Mọi thống kê/lọc theo danh mục cha phải cộng gộp cả giao dịch của mọi danh mục con (dùng một hàm dùng chung, không lặp điều kiện lọc ở nhiều nơi)
- [x] **TXN-09
...
| WALLET-01 | Phase 2 | Done |
| WALLET-02 | Phase 2 | Done |
| WALLET-03 | Phase 2 | Done |
| WALLET-04 | Phase 2 | Done |
| WALLET-05 | Phase 2 | Done |
| WALLET-06 | Phase 2 | Done |
| WALLET-07 | Phase 2 | Done |
| WALLET-08 | Phase 2 | Done |
| CAT-01 | Phase 2 | Done |
| CAT-02 | Phase 2 | Done |
| CAT-03 | Phase 2 | Done |
| CAT-04 | Phase 2 | Done |
